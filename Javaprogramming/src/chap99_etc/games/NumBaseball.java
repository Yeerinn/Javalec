...
public class NumBaseball {
	...
	public void start() {	
		...
		//3. 사용자 입력 값 3개 받아서 user배열에 저장 gameCnt++; 회수 증가
		//3스트라이크가 될 때까지 반복되도록 while 무한루프로 감싸줌
		while(true) {
			...
			//judgeStrike 메소드에서 strike가 3이상일 걍우
	...
	public void generateRandom() {
		for(int i = 0; i < com.length; i++) {
			...
			//중복체크
			//checkUpilcate() 메소드의 겨로가가 true일 때
			if(i < 0) {
	...
	//스트라이크/볼 판정 메소드, 스트라이크/볼 출력
	//스트라이크 개수로 계속 입력인지 게임종료인지 판단해주는 메소드(리턴 값 boolean으로 true, false) 
	//strike < 3면 false; else return true;
	//judgeStrike
