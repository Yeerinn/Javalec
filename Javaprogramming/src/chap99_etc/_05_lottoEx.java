...
public class _05_lottoEx {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		//당첨번호 담아줄 배열 6개 + 보너스번호 1개
				//인젣스가 0~5 : 로또번호, 인덱스 6 : 보너스 번호
				...
				for(int i =0; i < userNum.length; i++) {
					System.out.println("1 ~ 45까지의 정수 중 하나를 입력하세요.(중복 불가)");
				...
				for(int i = 0; i < lottoNum.length; i++) {
					for(int j = 0; j < userNum.length; j++) {
						//로또번호 인덱스가 i인 값 한개와 사용자번호 0~5까지의 6개랑 전부 비교
						if(lottoNum[i] == userNum[i]) {
							//인덱스가 6(보너스번호)가 맞았는지
							if(i == 6) {
								bonus = true;
							} else {
								//보너스 번호를 제외한 번호가 맞았는지
								correctCnt++;
							}
						}
					}
